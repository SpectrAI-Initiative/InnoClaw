...
#### 2026-07-03
- **通用 Kubernetes 任务调度器**: 使 AI 代理能够使用可配置的任务配置文件在 Kubernetes 集群上定义、验证和执行容器化工作负载


#### 2026-06-26
- **CLI运行时支持**: 支持REPL、批量操作和代理集成的交互式命令行界面，用于基于终端的工作流
...
#### 2026-06-12
- **禁用认证模式**: 在无身份验证的情况下运行应用程序，适用于单用户或开发部署场景




#### 2026-05-12
- **本地用户认证**: 新增内置账号注册、登录、退出登录，以及持久会话支持。
- **管理员用户管理**: 新增管理员用户页面，可创建账号并管理角色、启用状态、密码和数据归属。
