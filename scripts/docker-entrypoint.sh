...
# The app runs Drizzle migrations during startup via src/lib/db/migrate.ts.
# Keep the entrypoint lightweight and avoid depending on a local CLI shim.
echo "==> Database migrations will run during app startup"
